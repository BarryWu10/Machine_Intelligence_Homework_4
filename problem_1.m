...
% regression
[cost_slow, grad_slow] = costFunctionLogisticRegression_slow(initial_theta, Xdata, y, lambda);
fprintf('Cost at initial theta (zeros) using slow cost function logistic regression: %f\n', cost_slow); %should be about 0.693
%fprintf('Gradient using slow cost function logistic regression: %f\n', grad_slow(:1));

% uses optimized cost function logisitc regression
[cost, grad] = costFunctionLogisticRegression(initial_theta, Xdata, y, lambda);
fprintf('Cost at initial theta (zeros) using fast cost function logistic regression: %f\n', cost); %should be about 0.693
%fprintf('Gradient using fast cost function logistic regression: %f\n', grad);