...
k_value = 3;
Idx = knnsearch(TrainDataCV,TestDataCV, 'k', k_value);
Idx_gt = [TrainDataGT(Idx(:,1)) TrainDataGT(Idx(:,2)) TrainDataGT(Idx(:,3))];
Idx_gt_T = Idx_gt';
TestDataPred=mode(Idx_gt_T)';
otherwise
error('Unknown classificatio method')
end
...
figure
title_string = ['method:', method,' {\lambda}=', num2str(lambda),', Accuracy=', num2str(accuracy * 100),'%'];
confusionchart(confusionMatrix, 'Title', title_string);
print -dpng hwk4_problem5_plot.png